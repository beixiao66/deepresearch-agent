...
<!--
截图补充说明（截完把对应 TODO 替换为图片引用，图片放到 docs/screenshots/）：

截图 1 - 知识库管理页（http://127.0.0.1:8080/knowledge-bases）
  前置：确保某个知识库（如 RAG评估语料库）里已有状态为"已完成"的文档
  画面应包含：左侧知识库列表、右侧文档表格（文件名/状态徽章"已完成"/大小/删除按钮）

截图 2 - 创建研究页（http://127.0.0.1:8080/research/create）
  画面应包含：研究主题输入框（可填示例主题）、知识库下拉框、
  "知识库不足时允许联网搜索"开关、"或上传文件直接研究"文件选择框

截图 3 - 研究执行页（http://127.0.0.1:8080/research/run/{taskId}）
  前置：先创建一个研究任务并确认计划，任务正在执行中
  画面应包含：顶部进度条（25%/50%/75%/100%）+ 四个阶段指示
  （生成计划/人工确认/检索资料/生成报告）、SSE 事件流（黑色终端样式）

截图 4 - 研究报告页（http://127.0.0.1:8080/research/report/{taskId}）
  前置：研究任务已完成
  画面应包含：Markdown 报告正文（结论/关键证据）、"参考来源"列表
  （[1] 文件名·片段 + 相关度 + 证据文本）、"Token 消耗统计"
  （总/输入/输出 Token 卡片 + 分段条形图）
-->

| 页面 | 截图 |
|------|------|
| 知识库管理（上传文档、状态展示） | <!-- TODO: 截图 1，见上方说明 --> |
| 创建研究（选择知识库 / 上传文件） | <!-- TODO: 截图 2，见上方说明 --> |
| 研究执行（计划确认 + SSE 进度） | <!-- TODO: 截图 3，见上方说明 --> |
| 研究报告（Markdown 渲染 + 参考来源 + Token 统计） | <!-- TODO: 截图 4，见上方说明 --> |
